...
final class HotKey {
	...
	init(action: @escaping () -> Void) {
		...
		InstallEventHandler(GetApplicationEventTarget(), { _, _, _ in
			NSLog("DevStack: hotkey pressed")
			DispatchQueue.main.async { HotKey.action?() }
			return noErr
		}, 1, &spec, nil, &handler)
		let id = EventHotKeyID(signature: OSType(0x4456_5354), id: 1)   // "DVST"
		let status = RegisterEventHotKey(UInt32(kVK_ANSI_D), UInt32(controlKey | optionKey), id, GetApplicationEventTarget(), 0, &ref)
		NSLog("DevStack: hotkey ⌃⌥D registration status %d", status)
	}
