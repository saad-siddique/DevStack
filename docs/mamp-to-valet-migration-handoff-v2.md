...
**Phase 4 — Batch** the remaining hosts **except `uncanny-automator`** (20 hosts: 24 real hosts minus the 3 pilots minus the protected site). — **14 of 20 DONE 2026-09-22** (Saad's pick: docs sites, all non-WP, automatorplugin, automator-app-dev, elearning-plugins, automator-plugin-platform, hrpartner, unito, lindris, uncannyowl). Remaining: `tincanny`, `tincanny-core`, `uncanny-ceu`, `uncanny-codes`, `uncanny-groups`, `uncanny-toolkit`. `bin/migrate-all` must skip protected sites unless called with `--include-protected`.
